...
class WithdrawSnowflakeBalanceFromVia extends Component {
    ...
    withdraw = async () => {
        try {
            ...
            let token = await myContract.methods.withdrawSnowflakeBalanceFromVia(this.state.einFrom, this.state.via, this.state.einTo, this.state.amount, this.state._bytes,).call()
            console.log(token)
        }
        catch (ex) {
            console.log(ex)
